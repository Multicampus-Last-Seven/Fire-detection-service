...
def detect(opt):
    ...
    parser.add_argument('--name', default='fire', help='save results to project/name')
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    # 20210515 ysc
                    #---------------------
                    # 화재 발생시 체크 : fire 가 0.6 이상인 경우 화재로 인식
                    #---------------------
                    #label = f'{names[int(cls)]} {conf:.2f}' # detected label name
                    label = names[int(cls)]

                    #print(f'    nomal {label}  : {conf :.2f}')
                    fire_labels =['fire','fire-n']
                    if label in fire_labels and  conf >= 0.6 :
                        fire_alarm(label, conf )
                    #else :
                    #    print(f'    nomal {label}  : {conf :.2f}')
